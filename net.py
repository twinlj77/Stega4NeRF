...
class Secret(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Sequential(       #(3,180,180)
            nn.Conv2d(3, 64, kernel_size=(5, 5), stride=(3, 3), ),  #(64,59,59)
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=3, stride=3),  #(64,19,19)
            nn.Conv2d(64, 32, kernel_size=(3, 3), stride=(3, 3), ),  #(32,6,6)
        )
        self.fc = nn.Sequential(
            nn.ReLU(),
            nn.Linear(32 * 6 * 6, 1000),
            nn.Linear(1000, D*180 * 180),
        )
